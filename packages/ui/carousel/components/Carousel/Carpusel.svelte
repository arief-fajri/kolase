...
<div class="sc-carousel__carousel-container">
  <div class="sc-carousel__content-container">
    {#if arrows}
      <slot name="prev" showPrevPage={methods.showPrevPage}>
        <div class="sc-carousel__arrow-container">
          <Arrow direction="prev" disabled={!infinite && currentPageIndex === 0} on:click={showPrevPage} />
        </div>
      </slot>
    {/if}
    <div class="sc-carousel__pages-window" bind:this={pageWindowElement}>
      <div
        class="sc-carousel__pages-container"
        use:swipeable={{ thresholdProvider: () => pageWindowWidth / 5 }}
    ...
    {#if arrows}
      <slot name="next" showNextPage={methods.showNextPage}>
        <div class="sc-carousel__arrow-container">
          <Arrow direction="next" disabled={!infinite && currentPageIndex === pagesCount - 1} on:click={methods.showNextPage} />
        </div>
      </slot>
    {/if}
  </div>
  {#if dots}
    <slot name="dots" {currentPageIndex} {pagesCount} showPage={handlePageChange}>
      <Dots {pagesCount} {currentPageIndex} on:pageChange={(event) => handlePageChange(event.detail)}></Dots>
    </slot>
  {/if}
</div>
